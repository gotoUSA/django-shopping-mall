"""
만료되었거나 오래전에 사용된 비밀번호 재설정 토큰을 정리하는 커맨드.

Cron 또는 Celery Beat 스케줄러에 등록하면 주기적으로 실행되어
데이터베이스 용량을 안정적으로 관리할 수 있다.
"""
...
class Command(BaseCommand):
    help = "만료되었거나 오래된 비밀번호 재설정 토큰을 정리합니다."
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            "1. 만료됐지만 사용되지 않은 토큰 "
            f"(생성 후 {timeout_seconds // 3600}시간 초과): {expired_unused_count}개"
        )
        self.stdout.write(
            f"2. 오래전에 사용된 토큰 (사용 후 {used_days}일 초과): {old_used_count}개"
        )
        ...
        if not dry_run:
            if total_count > 0:
                ...
                self.stdout.write(
                    "만료됐지만 사용되지 않은 토큰: "
                    f"{expired_unused_deleted[0]}개"
                )
                self.stdout.write(f"오래전에 사용된 토큰: {old_used_deleted[0]}개")
                self.stdout.write(
                    self.style.SUCCESS(
                ...
                self.stdout.write(self.style.SUCCESS("✓ 삭제할 토큰이 없습니다."))
        else:
            self.stdout.write(
                self.style.WARNING("DRY RUN 모드: 실제 삭제는 수행하지 않았습니다.")
            )
            self.stdout.write("실제 삭제를 하려면 --dry-run 옵션을 제거하세요.")
