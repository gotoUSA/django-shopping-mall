...
class TestOrderStatusHappyPath:
    ...
    def test_filter_by_pending_status(self, authenticated_client, user, order_factory):
        """pending 상태 필터링"""
        # Arrange
    ...
    def test_filter_by_paid_status(self, authenticated_client, user, order_factory):
        """paid 상태 필터링"""
        # Arrange
        order_factory(user, status="pending", total_amount=Decimal("10000"))
        order_factory(user, status="paid", total_amount=Decimal("20000"))
    ...
    def test_status_display_in_list(self, authenticated_client, user, order_factory):
    ...
    def test_status_display_in_detail(self, authenticated_client, user, order_factory, order_statuses):
    ...
    def test_can_cancel_for_cancelable_statuses(self, authenticated_client, user, order_factory, order_statuses):
        """취소 가능한 상태의 can_cancel=True 확인"""
        # Arrange - pending, paid는 취소 가능
        for status_value in order_statuses["cancelable"]:
            order = order_factory(user, status=status_value, total_amount=Decimal("10000"))
            url = reverse("order-detail", kwargs={"pk": order.id})
    ...
    def test_can_cancel_for_non_cancelable_statuses(self, authenticated_client, user, order_factory, order_statuses):
        """취소 불가능한 상태의 can_cancel=False 확인"""
        # Arrange
        for status_value in order_statuses["non_cancelable"]:
            order = order_factory(user, status=status_value, total_amount=Decimal("10000"))
            url = reverse("order-detail", kwargs={"pk": order.id})
...
class TestOrderStatusBoundary:
    ...
    def test_empty_status_filter(self, authenticated_client, user, order_factory):
        """빈 문자열 상태 필터"""
        # Arrange
        order_factory(user, status="pending", total_amount=Decimal("10000"))
    ...
    def test_whitespace_status_filter(self, authenticated_client, user, order_factory):
        """공백 상태 필터"""
        # Arrange
        order_factory(user, status="pending", total_amount=Decimal("10000"))
        url = reverse("order-list") + "?status= "
    ...
    def test_case_sensitive_status_pending(self, authenticated_client, user, order_factory):
    ...
    def test_case_sensitive_status_paid(self, authenticated_client, user, order_factory):
        """대소문자 구분 - PAID (전체 대문자)"""
        # Arrange
        order_factory(user, status="paid", total_amount=Decimal("10000"))
        url = reverse("order-list") + "?status=PAID"
    ...
    def test_nonexistent_status_value(self, authenticated_client, user, order_factory):
        """존재하지 않는 상태값"""
        # Arrange
        order_factory(user, status="pending", total_amount=Decimal("10000"))
    ...
    def test_multiple_status_filter(self, authenticated_client, user, order_factory):
        """복수 상태 필터링 시도"""
        # Arrange
        order_factory(user, status="pending", total_amount=Decimal("10000"))
        order_factory(user, status="paid", total_amount=Decimal("20000"))
    ...
    def test_filter_with_no_matching_orders(self, authenticated_client, user, order_factory):
        """해당 상태 주문이 없는 경우"""
...
class TestOrderStatusException:
    ...
    def test_sql_injection_attempt(self, authenticated_client, user, order_factory):
        """SQL Injection 시도"""
        # Arrange
        order_factory(user, status="pending", total_amount=Decimal("10000"))
    ...
    def test_special_characters_in_status(self, authenticated_client, user, order_factory):
        """특수문자 포함 상태값"""
        # Arrange
        order_factory(user, status="pending", total_amount=Decimal("10000"))
        url = reverse("order-list") + "?status=pending!@#$%"
    ...
    def test_very_long_status_value(self, authenticated_client, user, order_factory):
        """매우 긴 상태값"""
        # Arrange
        order_factory(user, status="pending", total_amount=Decimal("10000"))
        long_status = "a" * 1000
    ...
    def test_unicode_characters_in_status(self, authenticated_client, user, order_factory):
        """유니코드 문자 포함 상태값"""
        # Arrange
        order_factory(user, status="pending", total_amount=Decimal("10000"))
